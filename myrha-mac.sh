...
if [[ -n "$MOSNAME" ]]; then
  ...
  if [[ -n "$MOS_STATUS_FILE" ]]; then
    ...
    echo "" >>"$OUT"
    # check_known_issues call was here, but we moved it up
    MOS_DOC_URL=""
...
if [[ -n "$MOSNAME" ]]; then
  ...
  while read -r line; do
    printf "# $line:" >>"$OUT"
    echo "" >>"$OUT"
    grep -iE 'error|fail|warn' "$line" | sed -E '/^\s*$/d' >>"$OUT"
    echo "" >>"$OUT"
...
if [[ -n "$MOSNAME" ]]; then
  ...
  while read -r line; do
    printf "# $line:" >>"$OUT"
    echo "" >>"$OUT"
    grep -E 'ERR|WARN' $line | sed -E '/^\s*$/d' | tail -n 150 >>"$OUT"
    echo "" >>"$OUT"
...
if [[ -n "$MOSNAME" ]]; then
  ...
  while read -r line; do
    printf "# $line:" >>"$OUT"
    echo "" >>"$OUT"
    grep -E '\[warning\]|\[error\]' $line | sed -E '/^\s*$/d' | tail -n 150 >>"$OUT"
    echo "" >>"$OUT"
...
if [[ -n "$MOSNAME" ]]; then
  ...
  grep -iE 'error|fail|warn' $MOS_DIR/objects/namespaced/openstack/core/pods/mariadb-controller-*/controller.log | sed -E '/^\s*$/d' >>"$OUT"
  echo "" >>"$OUT"
...
if [[ -n "$MCCNAME" ]]; then
  ...
  grep -iE 'error|fail|warn' $MCC_DIR/objects/namespaced/kaas/core/pods/mariadb-controller-*/controller.log | sed -E '/^\s*$/d' >>"$OUT"
  echo "" >>"$OUT"
  echo "## Logs from server-0 pods (Errors/Warnings):" >>"$OUT"
  printf '# ' >>"$OUT"
...
if [[ -n "$MCCNAME" ]] || [[ -n "$MOSNAME" ]]; then
  ...
  for f in "$LOGPATH"/*cluster.yaml; do
    ...
    # 1. Stuck message (full) - Extract using yq from the normalized file
    STUCK=$(yq eval '.lcmOperationStuckMessage // .status.lcmOperationStuckMessage' "$f" 2>/dev/null)
    if [[ -n "$STUCK" && "$STUCK" != "null" ]]; then
       echo "  lcmOperationStuckMessage: $STUCK" >>"$OUT"
    fi
...
EOF
  ...
  open "$HTML_REPORT" 2>/dev/null || open "$HTML_REPORT" 2>/dev/null
